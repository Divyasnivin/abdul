...
async def iq(iqthonevent):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "Welcome Source Jack •"
...
• : ping  {ping}  𓇡.
• : version 7.6  𓇡.
• : Source Jack : @kbbbd2  𓇡."""
@iqthon.on(admin_cmd(pattern="التنصيب(?: |$)(.*)"))    
async def source(e):
    await edit_or_reply(e, "https://github.com/TelethonArab/TelethonAr",)
@iqthon.on(admin_cmd(pattern="حساب قيثاب( -l(\d+))? ([\s\S]*)"))    
async def _(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(URL) as request:
            ...
            catevent = await edit_or_reply(event, "**•︙  جـاري إحضـار معلومـات حساب قيثاب ↯**")
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**•︙  📑 يرجى متابع قناه الفارات تجدها هنا : @kbbbd2")
...
async def psu(event):
    ...
    cpufreq = psutil.cpu_freq()
    for i, percentage in enumerate(psutil.cpu_percent(percpu=True)):
        cpuu = "**حجم استخدامك جاك :**\n"
    cpuu += f"الاستخدام : `{psutil.cpu_percent()}%`\n"
    svmem = psutil.virtual_memory()
    help_string = f"{str(cpuu)}\n"
    await event.edit(help_string)
@iqthon.on(admin_cmd(pattern="سرعه النت(?:\s|$)([\s\S]*)"))    
async def _(event):
    ...
    if input_str == "image":
        ...
        as_text = True
    catevent = await edit_or_reply(event, "**•︙   جـاري حسـاب سرعـه الانترنت لـديك  **")
    start = time()
    ...
    try:
        ...
        if as_text:
            await catevent.edit(                """**•︙   حسـاب سرعـه الانترنت لـديك  : {} ثانية**
...
**•︙   تقيم الانترنيت :** `{}`""".format(                    ms,                    convert_from_bytes(download_speed),                    round(download_speed / 8e6, 2),                    convert_from_bytes(upload_speed),                    round(upload_speed / 8e6, 2),                    ping_time,                    i_s_p,                    i_s_p_rating,                )            )
        ...
        await catevent.edit(            
"""**•︙   حسـاب سرعـه الانرنت لـديك  : {} ثانية**
**•︙   التنزيل :** `{} (or) {} ميغا بايت`
**•︙   الرفع :** `{} (or) {} ميغا بايت`
**•︙   البنق :** {}` بالثانية`
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineiqthon(iqthon):
        ...
        query = iqthon.text
        await bot.get_me()
        if query.startswith("تةةةاااىاتةنصيب") and iqthon.query.user_id == bot.uid:
...
@bot.on(admin_cmd(outgoing=True, pattern="تنااااااااصيب"))
async def repoiqthon(iqthon):
    ...
    response = await bot.inline_query(TG_BOT, "تنتتااعتتصيب")
...
async def iq(event):
    await edit_or_reply(event, "*تم فتح الهمسة  **\n** روح للرسائل المحفوظة وبتلقاها**")
