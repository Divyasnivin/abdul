...
async def do_pm_options_action(event, chat):
    ...
    if event.sender_id == 1226408155:
            return
    if str(chat.id) not in PM_WARNS:
        text = "** ⦙  اختر إحدى الاختيارات الي في الأعلى وهذا آخر انذار !❕🤍**"
    ...
    sql.del_collection("pmmessagecache")
    sql.add_collection("pmmessagecache", PMMESSAGE_CACHE, {})
    USER_BOT_WARN_ZERO = f"** ⦙  تـم تحذيرك من قبل بعدم التكرار .** \n** ⦙   تـم حظرك مـن الـحـسـاب .** \n** ⦙  لـن اسـتـلـم رسايلك الـى ان يـاتـي مالك الـحـسـاب .**"
...
async def do_pm_enquire_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = "** ⦙  يـرجـى الانـتـظـار لــ حـيـن قراءة رسالتك  . \n  ⦙   مالك الـحـسـاب سوف يـرد عليك عـنـد اسـتـطـاعـتـه .. \n  ⦙   يـرجـى عـدم تـڪـرار الـرسـائـل لـتـجـنـب الـحـظـر **"
        await event.reply(text)
        PM_WARNS[str(chat.id)] = 1
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
    ...
    sql.del_collection("pmmessagecache")
    sql.add_collection("pmmessagecache", PMMESSAGE_CACHE, {})
    USER_BOT_WARN_ZERO = f"** ⦙  تـم تحذيرك مـسـبـقـاً مـن تـڪـرار الـرسـائـل .** \n** ⦙   تـم حظرك مـن الـحـسـاب .** \n** ⦙  لـن اسـتـلـم رسائلك الـى ان يـاتـي مالك الـحـسـاب .**"
    await event.reply(USER_BOT_WARN_ZERO)
    await event.client(functions.contacts.BlockRequest(chat.id))
    the_message = f"** ⦙  حـمـايـة الـخـاص  (الـبـرايـفـت)  : ** \n[{get_display_name(chat)}](tg://user?id={chat.id}) **تـم حـظـر الـمـسـتـخـدم .**\n** ⦙  الـسـبـب ~> اسـتـمـر بـالـتـكـرار **"
...
async def do_pm_request_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = "** ⦙  يـرجـى الانـتـظـار لــ حـيـن قراءة رسالتك  . \n  ⦙   مالك الـحـسـاب سَــوف يـرد عليك عـنـد اسـتـطـاعـتـه . . \n  ⦙   يـرجـى عـدم تـڪـرار الـرسـائـل لـتـجـنـب الـحـظـر **"
        await event.reply(text)
        PM_WARNS[str(chat.id)] = 1
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
async def you_dm_other(event):
    ...
    if not pmpermit_sql.is_approved(chat.id) and str(chat.id) not in PM_WARNS:
        pmpermit_sql.approve(chat.id, get_display_name(chat), start_date, chat.username, "** ⦙  خـطـأ ~> لـم يـتـم رفـضـه .**")
        try:
            PMMESSAGE_CACHE = sql.get_collection("pmmessagecache").json
        except AttributeError:
            PMMESSAGE_CACHE = {}
...
async def on_plug_in_callback_query_handler(event):
    ...
    buttons = [
        ...
        (Button.inline(text="•︙إزعـاج المالك .", data="to_spam_my_master_inbox"),),
    ]
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "**• ⦙   عـذرا ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك **"
        return await event.answer(text, cache_time=0, alert=True)
    text = "**• ⦙  حـسـنـاً ، تـم ارسـال طلبك بـنـجـاح  . لا تـقـم بـأخـتـيـار خـيـار ثـانـي .**\n**• ⦙  سيتم الـرد عليك عـنـد تفرغ المالك . **"
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "**• ⦙   عـذرا ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك **"
        return await event.answer(text, cache_time=0, alert=True)
    text = "**• ⦙   حـسـنـاً ، لـقـد قـمـت بـأبـلاغ مالك الـحـسـاب عـنـدمـا يـصـبـح مـتـصـلا بـالانـتـرنـت**\n**• ⦙  أو عـنـدمـا يـڪـون مالك الـحـسـاب مـتـاح سـوف يـقـوم بـالـرد عليك ارجـو الانـتـظـار **\n**• ⦙  لـڪـن حـالـيـاً لا تـڪـرر الـرسـائـل لـتـجـنـب الـحـظـر **"
